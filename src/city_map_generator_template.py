...
# City configuration
CITY_NAME = "Your City"  # Name of your city (used for output filenames)
...
NUM_DISTRICTS = 4  # Number of districts to create

# Ensemble configuration
TEST_ENSEMBLE_SIZE = int(input("Enter the size of test ensemble to generate statistics (default 100): ") or 100)  # Size of test ensemble to generate statistics
MAIN_ENSEMBLE_SIZE = int(input("Enter the size of main ensemble for final analysis (default 1000): ") or 1000)  # Size of main ensemble for final analysis
TOP_N_PLANS = int(input("Enter the number of top plans to save block assignment CSVs for (default 100): ") or 100)  # Number of top plans to save block assignment CSVs for
...
POPULATION_EPSILON = 0.01  # Population deviation tolerance (2% = 0.02)
...
def generate_plan_without_scoring(graph, num_districts, index):
    """Generate a plan without scoring (for test ensemble)."""
    timings = {}
    t0 = time.time()
...
def generate_and_save_plan(graph, num_districts, index, timing_log):
    ...
    try:
        ...
        if "Could not find a possible cut" in str(e):
            print(f"Plan {index+1}: Skipping due to partitioning failure - {e}")
            return None
        else:
            raise e
    ...
    plan_data.update(metrics)
    timings['total_time'] = time.time() - t0
    timings['plan_number'] = index + 1
    timing_log.append(timings)

    # --- Calculate and print plan score here ---\
    row = pd.Series(plan_data)
    score_arr = plan_score_array(row)
    plan_score = weighted_geom_mean(score_arr)
    plan_data["score_array"] = score_arr
    plan_data["plan_score"] = plan_score
    print(f"Plan {index+1} score: {plan_score:.4f}")
...
def main():
    ...
    top_plan_scores_sorted = sorted(top_plan_scores, key=lambda x: -x[0])
